...
Rcpp::NumericVector simulate_cfp(
    const otos::parameter<otos::cfp<double, 2>>& p
  , std::size_t n, int seed) {

  Eigen::VectorXd out = Eigen::VectorXd::Zero(n);
  auto model = otos::cfp<double, 2>(p);
...
Rcpp::NumericVector filter_cfp(
  ...
  otos::cfp<double, 2> model(params);
...
otos::parameter<cfp<double, 2>> em_cfp(
    const param_type& params
  , const Rcpp::NumericVector& col
  , std::size_t nstep
  , double tol) {
  ...
  const auto in = Rcpp::as<std::vector<double>>(col);
  parameter<otos::cfp<double, 2>> out;

  cfp<double, 2> model(params);

  model.emax2<otos::recorders::type::none>(
    Eigen::Map<const Eigen::VectorXd>(in.data(), in.size())
    , out, nstep, tol);
...
Rcpp::DataFrame em_cfp_debug(
  ...
  std::size_t nrows = col.size();
  const auto in = Rcpp::as<std::vector<double>>(col);
  otos::parameter<otos::cfp<double, 2>> out;
  otos::cfp<double, 2> model(params);

  auto diagnostics = model.emax2<otos::recorders::type::simple>(
    Eigen::Map<const Eigen::VectorXd>(in.data(), in.size())
    , out, nstep, tol);
